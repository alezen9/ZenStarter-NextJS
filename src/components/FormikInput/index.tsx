...
export const IGNORE_OPTION = {
   value: '_ignoreField',
   label: '### Ignora questo campo ###',
   component: <span style={{ color: 'crimson' }}>### Ignora questo campo ###</span>
}
...
const FormikInput = (props: Props) => {
   ...
   const defaultProps = { ...props, formik: formikProps, id, type, values, handleChange, helperText, errors, variant, disabled, xs, multiline, rows, required, label }
   ...
   switch (type) {
      ...
      case 'checkbox':
         const onChangeCheckbox = (e, d: boolean) => {
            _setFieldValueTouched(name, d)
            if (supplementaryOnChange) supplementaryOnChange(e, d)
         }
         return (
            <GridWrapper {...props}>
               <FormControlLabel
                  classes={{ label: checkSwitchLabel }}
      ...
      case 'asyncAutocomplete':
         const onChangeAsyncAutocomplete = (e, d) => {
            // if (props.multiple) {
            //    const n = (d || []).length
            //    const val = !!n ? d[n - 1] : null
            //    const currentVals = get(values, name, [])
            //    const newVals = compact(uniqBy([...currentVals, val], 'value'))
            //    console.log(currentVals, newVals, d)
...
const GridWrapper = React.memo((props: GridWrapperProps) => {
   ...
   return <Grid className={classes.gridWrapper} item {...container && { container: true, spacing: spacing || 1 }} style={{ ...props.gridStyle || {} }} xs={xs} sm={sm || xs} md={md || sm || xs} lg={lg || md || sm || xs} xl={xl || lg || md || sm || xs}>
      {props.children || <></>}
   </Grid>
})
